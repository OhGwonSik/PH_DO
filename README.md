...
- **문제**: 크레인마다 담당 케파(로케이션 범위)가 정해져 있는데, 기존 설계에는 로케이션-크레인 매핑 관계가 반영되지 않아 실제로는 처리 불가능한 조합도 선택 가능했음 (개발 중 직접 발견)
- **해결**: 로케이션↔크레인 매핑 관리 화면을 직접 제안 및 구현 — 선별 시 한쪽을 지정하면 매핑된 값만 노출되도록 양방향 필터링 처리해 처리 불가능한 조합 선택을 사전 차단
